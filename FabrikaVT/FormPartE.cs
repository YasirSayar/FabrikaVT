...
namespace FabrikaVT
{
    public partial class FormPartE : Form
    {
        ...
        private void FormPartE_Load(object sender, EventArgs e)
        {
            try
            {
           ...
           string query = "SELECT Part.part_name as Parça, Part.part_quantity as Adet, Suplier.sup_name as [Tedarikçi Adı], Part.part_id FROM Part INNER JOIN Suplier ON Part.suplier_id = Suplier.suplier_id;";
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string urunDeger = selectedRow.Cells["Parça"].Value.ToString();
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                using (SqlConnection connection = new SqlConnection(baglantiObj.getConnection()))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("UPDATE Part SET part_quantity = @Param1 WHERE part_id= @id", connection))
                    {
                        command.Parameters.AddWithValue("@Param1", textBoxQuantity.Text);
                        command.Parameters.AddWithValue("@id", part_id);
            ...
            if (textBoxQuantity.Text != string.Empty || comboBox1.Text != string.Empty)
            {
                try
                {
                    ...
                    using (SqlConnection connection = baglantiObj.CreateConnection())
                    {
                        ...
                        string query = "SELECT Part.part_name as Parça, Part.part_quantity as Adet, Suplier.sup_name as [Tedarikçi Adı], Part.part_id FROM Part INNER JOIN Suplier ON Part.suplier_id = Suplier.suplier_id where (Part.part_name LIKE @Param1 OR @Param1 IS NULL) AND (part_quantity LIKE @Param2 OR @Param2 IS NULL)";
